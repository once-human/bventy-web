...
import { QuoteContact, messagingService, Conversation } from "@bventy/services";
        ...
        <div className="flex min-h-screen flex-col bg-slate-50 dark:bg-slate-950">
            ...
            <main className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
                ...
                <Dialog open={isDetailsOpen} onOpenChange={(open) => {
                    ...
                    if (!open) setTimeout(() => setShowChat(false), 200); // reset state smoothly
                }}>
                    <DialogContent className={`sm:max-w-[500px] p-0 overflow-hidden ${showChat ? 'sm:max-w-[700px]' : ''}`}>

                        {showChat ? (
                            <div className="flex flex-col h-[650px] bg-slate-50 dark:bg-slate-950">
                                <div className="flex items-center justify-between p-4 border-b bg-card">
                                    <div className="flex items-center gap-3">
                                        <Button variant="ghost" size="icon" onClick={() => setShowChat(false)} className="h-8 w-8">
                                            <ArrowLeft className="h-4 w-4" />
                                        </Button>
                                        <DialogTitle>Chat with {selectedQuote?.vendor_name}</DialogTitle>
                                    </div>
                                </div>
                                <div className="flex-1 overflow-hidden p-4 rounded-b-lg">
                                    {conversations.find(c => c.quote_id === selectedQuote?.id) ? (
                                        <ChatInterface
                                            conversationId={conversations.find(c => c.quote_id === selectedQuote?.id)!.id}
                                            currentUserId={user!.id}
                                            chatLocked={conversations.find(c => c.quote_id === selectedQuote?.id)!.chat_locked}
                                            otherPartyName={selectedQuote.vendor_name || "Vendor"}
                                        ...
                                        <div className="h-full flex items-center justify-center text-muted-foreground border border-dashed rounded-lg">
                                            Chat is not available for this legacy quote.
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <>
                                ...
                                <div className="p-6 pt-0">
                                    ...
                                    {selectedQuote?.status !== 'accepted' && conversations.find(c => c.quote_id === selectedQuote?.id) && (
                                        <div className="mt-4 border-t pt-4">
                                            <Button variant="secondary" className="w-full" onClick={() => setShowChat(true)}>
                                                <MessageCircle className="h-4 w-4 mr-2" />
                                                Open Negotiator Chat
                                            </Button>
                                        </div>
                                    )}
                                </div>
