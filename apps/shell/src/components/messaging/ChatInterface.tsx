...
export function ChatInterface({ conversationId, currentUserId, chatLocked, otherPartyName, otherPartyRole, otherPartyId, otherPartyOnline, otherPartySubtitle, quoteId, quoteStatus, onQuoteResponded }: ChatInterfaceProps) {
    ...
    const [isOtherUserOnline, setIsOtherUserOnline] = useState(otherPartyOnline || false);

    useEffect(() => {
        setIsOtherUserOnline(otherPartyOnline || false);
    }, [otherPartyOnline]);
    ...
    useEffect(() => {
        ...
        if (lastMessage.type === 'new_message') {
            ...
            const { user_id, online } = lastMessage.payload;
            if (user_id === otherPartyId) {
                setIsOtherUserOnline(online);
            }
        }
    }, [lastMessage, conversationId, currentUserId, otherPartyId]);
        ...
        <div className="flex flex-col h-full border border-border rounded-lg bg-card overflow-hidden shadow-sm">
            ...
            <div className="flex items-center justify-between p-4 border-b border-border bg-card">
                <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-medium text-lg border border-primary/5">
                        {otherPartyName.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex flex-col">
                        <h3 className="font-semibold text-foreground/90 leading-tight">{otherPartyName}</h3>
                        <div className="flex items-center gap-1.5 mt-0.5">
                            <span className="text-[11px] text-primary/70 font-medium capitalize">{otherPartySubtitle || otherPartyRole}</span>
                            <span className="text-[10px] text-muted-foreground/40">•</span>
                            <div className="flex items-center gap-1">
                                <span className={`h-1.5 w-1.5 rounded-full ${isOtherUserOnline ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.4)]' : 'bg-muted-foreground/30'} transition-all`} />
                ...
                <div className="flex items-center gap-1">
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 rounded-xl">
                            ...
                            <DropdownMenuItem className="text-sm cursor-pointer py-2 text-destructive focus:text-destructive">
                                Block User
                            </DropdownMenuItem>
                            <DropdownMenuItem className="text-sm cursor-pointer py-2 text-destructive focus:text-destructive">
                                Delete Chat
                                ...
                                <div className={`flex flex-col ${isSystem ? 'items-center my-6' : isMe ? 'items-end' : 'items-start'}`}>
                                    ...
                                    {isSystem ? (
                                        ...
                                        <div className={`flex flex-col max-w-[85%] sm:max-w-[70%] ${isMe ? 'items-end' : 'items-start'}`}>
                                            ...
                                            <div className={`flex items-center gap-1 mt-1 px-1 text-[10px] text-muted-foreground font-medium`}>
                                                <span>{hasMounted ? format(new Date(msg.created_at), 'h:mm aa') : '--:--'}</span>
                                                {isMe && (
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center gap-2 bg-muted/30 p-1.5 rounded-xl border border-border/50">
                                <Button
                                    ...
                                    size="icon"
                                    variant="ghost"
                                    className="shrink-0 h-10 w-10 rounded-lg text-muted-foreground hover:text-foreground hover:bg-background/80"
                                    title="Attach file"
                                    onClick={() => fileInputRef.current?.click()}
                                ...
                                <form onSubmit={handleSend} className="flex-1 flex gap-2">
                                    <Input
                                        ...
                                        className="flex-1 h-10 rounded-lg px-2 border-none focus-visible:ring-0 bg-transparent text-sm placeholder:text-muted-foreground/50 transition-all shadow-none"
                                        disabled={isSending}
                                    />
                                    <Button
                                        type="submit"
                                        size="icon"
                                        className="shrink-0 h-10 w-10 rounded-lg transition-transform active:scale-95 bg-primary hover:bg-primary/90 shadow-md"
